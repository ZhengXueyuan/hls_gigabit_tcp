...
module wrapper_min (
    ...
    input  [3:0]    phy1_rxd,
    input           phy1_rxctl,
    // RGMII TX to PHY (inout for pad-level loopback sampling)
    inout           phy1_txc,
    inout [3:0]     phy1_txd,
    inout           phy1_txctl,
    // UART
    ...
    wire [3:0] rgmii_td_int;
    wire       rgmii_tx_ctl_int;
    wire       rgmii_txc_int;
    wire [3:0] phy1_txd_pad;
    wire       phy1_txctl_pad;
    wire       phy1_txc_pad;

    util_gmii_to_rgmii u_rgmii (
        .reset          (1'b0),
        .rgmii_td       (rgmii_td_int),
        .rgmii_tx_ctl   (rgmii_tx_ctl_int),
        .rgmii_txc      (rgmii_txc_int),
        .rgmii_rd_i     (phy1_rxd),
    ...
    // --- PAD LOOPBACK: IOBUF (T=0) on TX pins, sample the PAD via the input path ---
    genvar g_io;
    generate
        for (g_io = 0; g_io < 4; g_io = g_io + 1) begin : gen_io_txd
            IOBUF #(.DRIVE(12), .IOSTANDARD("LVCMOS18"), .SLEW("FAST"))
                u_io_txd ( .I(rgmii_td_int[g_io]), .O(phy1_txd_pad[g_io]),
                           .T(1'b0), .IO(phy1_txd[g_io]) );
        end
    endgenerate
    IOBUF #(.DRIVE(12), .IOSTANDARD("LVCMOS18"), .SLEW("FAST"))
        u_io_txctl ( .I(rgmii_tx_ctl_int), .O(phy1_txctl_pad), .T(1'b0), .IO(phy1_txctl) );
    IOBUF #(.DRIVE(12), .IOSTANDARD("LVCMOS18"), .SLEW("FAST"))
        u_io_txc   ( .I(rgmii_txc_int),   .O(phy1_txc_pad),   .T(1'b0), .IO(phy1_txc) );
    ...
    // --- PIN ACTIVITY SAMPLER (?raw report): counts in 1-second windows ---
    // raw_cap layout (printed MSB first by net_stats, hex pairs):
    //   [127:80] zeros, [79:64]=txctl_hi_cycles, [63:48]=txd_change_cycles,
    //   [47:32]=txc_ones(sat. 0xFFFF=active), [31:16]=frames_in_window,
    //   [15:0]=txc_edges (sat.)
    // Sampled from the pre-OBUF nets (phy1_txc etc.) — proves the
    // fabric actually drives the IOBs.
    reg [27:0] pin_txc_ones;
    reg [27:0] pin_txc_edges;
    reg [15:0] pin_txctl_hi;
    reg [15:0] pin_txd_chg;
    reg [15:0] pin_frames;
    reg [3:0]  pin_txd_prev;
    reg [15:0] pin_win;
    reg        pin_last_txc;
    reg [127:0] pin_report;
    always @(posedge gmii_clk or negedge reset_n) begin
        if (!reset_n) begin
            pin_txc_ones<=0; pin_txc_edges<=0; pin_txctl_hi<=0; pin_txd_chg<=0;
            pin_frames<=0; pin_txd_prev<=0; pin_win<=0; pin_last_txc<=0; pin_report<=0;
        end else begin
            pin_win <= pin_win + 16'd1;
            if (phy1_txc_pad) begin
                pin_txc_ones <= (pin_txc_ones == 28'hFFFFFFF) ? pin_txc_ones : pin_txc_ones + 28'd1;
                if (!pin_last_txc) pin_txc_edges <= (pin_txc_edges == 28'hFFFFFFF) ? pin_txc_edges : pin_txc_edges + 28'd1;
            end
            pin_last_txc <= phy1_txc_pad;
            if (phy1_txctl_pad) begin
                pin_txctl_hi <= (pin_txctl_hi == 16'hFFFF) ? pin_txctl_hi : pin_txctl_hi + 16'd1;
                if (!pin_prev_ctl) pin_frames <= (pin_frames == 16'hFFFF) ? pin_frames : pin_frames + 16'd1;
            end
            if (phy1_txd_pad != pin_txd_prev)
                pin_txd_chg <= (pin_txd_chg == 16'hFFFF) ? pin_txd_chg : pin_txd_chg + 16'd1;
            pin_txd_prev <= phy1_txd_pad;
            if (pin_win == 16'hFFFF) begin   // ~1s @125MHz
                pin_win <= 0;
                pin_report <= {48'h0, pin_txctl_hi, pin_txd_chg,
                               (pin_txc_ones[27:16] != 12'hFFF) ? 16'hFFFF : pin_txc_ones[15:0],
                               pin_frames, pin_txc_edges[15:0]};
                pin_txc_ones<=0; pin_txctl_hi<=0; pin_txd_chg<=0; pin_frames<=0;
                pin_txc_edges<=0;
            end
        end
    end
    reg pin_prev_ctl;
    always @(posedge gmii_clk or negedge reset_n) begin
        if (!reset_n) pin_prev_ctl <= 1'b0;
        else          pin_prev_ctl <= phy1_txctl_pad;
    end
