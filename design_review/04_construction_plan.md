...
> 状态: **调查完成, 待用户确认后开工** (前置决策见 §6)。本阶段未改任何代码。
...
3. **物理层前置**: 换 156.25MHz 晶振后走 PG157 自建 10G MAC+BASE-R PCS/PMA (GTX X0Y0)。
...
**决策选项**:
- **(a) 换晶振** (采购 SIT9120AI-2B3-33E156.25 + 焊接) → 走 10GBASE-R 全计划 (P0-P5)
- **(b) 不换晶振** → 降级为 1G RGMII + 同一新架构 (收益: 新架构全部保留, 带宽 10×损失, 但
  RGMII 1G 下每字节预算宽松 8 倍, 时序风险更小; 后续换晶振可原位升级 10G)

## 7. 分阶段施工计划 (~40-60 人天)

| 阶段 | 内容 | 验证手段 | 人天 | 里程碑意义 |
|---|---|---|---|---|
| **P0 物理层** | 换 156.25MHz 晶振 → 自建 PG157 (10G MAC+PCS, X0Y0), 修 XDC (MGTREFCLK 6.4ns / DRP_CLK G22); GTX 先回环 | 板级 ILA TX/RX 比对, 无需对端 | 5-8 | 物理链路成立 |
