...
module wrapper_min (
    ...
    // REGISTERED OUTPUT STAGE (experiment): mimic the demo's gmii_arbi
    // registered pipeline — e_txen/e_txd come from FFs, not the
    // combinational demo_out mux. Tests the "combinational source breaks
    // TX" hypothesis.
    reg e_txen_r;
    reg [7:0] e_txd_r;
    always @(posedge gmii_clk or negedge reset_n) begin
        if (!reset_n) begin e_txen_r <= 1'b0; e_txd_r <= 8'h00; end
        else begin e_txen_r <= demo_sending; e_txd_r <= demo_out; end
    end
    assign e_txd  = e_txd_r;
